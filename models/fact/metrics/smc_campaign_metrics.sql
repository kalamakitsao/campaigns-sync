...
-- Keep only the most recent form per (cycle_name, patient_id)
latest_cwc AS (
  SELECT *
  FROM (
    SELECT
...
-- Picking only the latest form per patient per cycle
campaigns AS (
  SELECT
    chp.chp_area_id,
    chp.county_name,
...
SELECT
  ...
  -- Referral (excluded due to fever or danger signs)
